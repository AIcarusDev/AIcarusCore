...
def test_normalize_action_payload_already_normalized():
...
async def test_dispatch_only_action(mock_dispatcher_dependencies: dict):
...
async def test_dispatch_only_consciousness_control(mock_dispatcher_dependencies: dict):
...
async def test_dispatch_both_action_and_control(mock_dispatcher_dependencies: dict):
...
async def test_dispatch_special_send_message_action(mocker: MockerFixture, mock_dispatcher_dependencies: dict):
    """测试 send_message 动作会走特殊的处理路径 (调用 MessageBuilder)。"""
    # 准备
    decision = {
        "internal_state": {"mood": "chatting", "think": "...", "intent": "reply"},
        "action": {"qq": {"send_message": {"steps": [{"command": "text", "params": {"content": "Hi"}}]}}}
    }
