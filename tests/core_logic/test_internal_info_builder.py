...
class TestInternalInfoBuilder:
    ...
    async def test_build_block_with_no_history(
        self, internal_info_builder: InternalInfoBuilder, mock_thought_storage_service: MockerFixture
    ):
    ...
    async def test_build_block_with_completed_state_and_real_action(
        ...
        assert '<completed_action><![CDATA[{"core": {"web_search": {"query": "pytest"}}}]]></completed_action>' in result
    ...
    async def test_build_block_with_interrupted_state(
        ...
        interrupting_stimulus = Stimulus(
            event_id="evt-123",
            timestamp=0, platform="test", bot_id="bot",
            text_content="<紧急消息&>", # 测试 XML 转义
            sender_id="user_abc"
        )
        mock_session.interruption_context = {
