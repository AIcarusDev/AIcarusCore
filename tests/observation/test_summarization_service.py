...
from src.config import config # 我们需要用到 config 里的 persona
...
class TestSummarizationService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_bot_profile = {
            'user_id': '12345',
            'nickname': '小懒猫测试版',
            'card': '摸鱼中'
        }
        self.mock_conversation_info = {
            'name': '小懒猫的测试群',
            'type': 'group',
            'id': 'group_1001'
        }
    ...
    def test_first_summary_generation(self):
        """
        ...
        self.mock_llm_client.make_llm_request.return_value = {
        ...
        new_events = [
            {
                '_key': 'event1', 'timestamp': 1700000000000, 'event_type': 'message.group.normal',
                'user_info': {'user_id': '67890', 'user_nickname': '用户A'},
                'content': [{'type': 'text', 'data': {'text': '这个单元测试好难啊！'}}]
            },
            {
                '_key': 'event2', 'timestamp': 1700000001000, 'event_type': 'action.message.send',
        ...
        result_summary = asyncio.run(self.service.consolidate_summary(
    ...
    def test_progressive_summary_generation(self):
        """
        ...
        self.mock_llm_client.make_llm_request.return_value = {
            "text": expected_new_summary,
            "error": None
        }
        
        new_events = [
            {
                '_key': 'event3', 'timestamp': 1700000002000, 'event_type': 'message.group.normal',
                'user_info': {'user_id': '67890', 'user_nickname': '用户A'},
    ...
    def test_llm_call_failure(self):
        ...
        new_events = [
            {
                '_key': 'event4', 'timestamp': 1700000003000, 'event_type': 'message.group.normal',
                'user_info': {'user_id': '67890', 'user_nickname': '用户A'},
