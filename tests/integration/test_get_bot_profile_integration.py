...
class TestGetBotProfileIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_real_bot_profile(self) -> None:
        """
        ...
        try:
            ...
            server = await websockets.serve(self.adapter_connection_handler, "localhost", self.test_server_port)
    ...
    async def adapter_connection_handler(self, websocket: websockets.WebSocketServerProtocol, path: str) -> None:
        """
        ...
        try:
            ...
            self.adapter_id_from_test = (
                reg_msg.get("content", [{}])[0].get("data", {}).get("details", {}).get("adapter_id")
            )
            display_name = (
                reg_msg.get("content", [{}])[0]
                .get("data", {})
                .get("details", {})
                .get("display_name", self.adapter_id_from_test)
            )
