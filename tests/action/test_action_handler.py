...
# 移除模块级别的 pytestmark = pytest.mark.asyncio
...
def mock_dependencies(mocker: MockerFixture) -> dict:
    ...
    mocks["web_search_agent_client"].make_llm_request.return_value = {
        "text": "mocked web search result"
    }
    return mocks
...
class TestActionHandlerHelpers:
    ...
    def test_resolve_safe_path_traversal_attack(
        ...
        def mock_commonpath(paths: list) -> str:
...
# 这个类中的测试都是异步的，所以给类加上 asyncio 标记
@pytest.mark.asyncio
class TestProcessActionFlowDispatch:
    """测试 process_action_flow 的分发逻辑."""

    @pytest.fixture
    def mock_action_handler_methods(
        self, action_handler_instance: ActionHandler, mocker: MockerFixture
    ) -> dict:
        """使用 mocker.spy 来监视内部方法的调用情况."""
        spies = {
            "_handle_do_nothing_action": mocker.spy(
                action_handler_instance, "_handle_do_nothing_action"
            ),
            "_handle_core_action_flow": mocker.spy(
                action_handler_instance, "_handle_core_action_flow"
            ),
            "_execute_platform_action_flow": mocker.spy(
                action_handler_instance, "_execute_platform_action_flow"
            ),
        }
        return spies

    async def test_dispatch_do_nothing(
        self, action_handler_instance: ActionHandler, mock_action_handler_methods: dict
    ) -> None:
        ...
        await action_handler_instance.process_action_flow(
            "action_id_1", "doc_key_1", action_json, metadata
        )
    ...
    async def test_dispatch_core_action(
        self,
        ...
        await action_handler_instance.process_action_flow(
            "action_id_2", "doc_key_2", action_json, metadata
        )
    ...
    async def test_dispatch_platform_action(
        ...
        await action_handler_instance.process_action_flow(
            "action_id_3", "doc_key_3", action_json, metadata
        )
        mock_action_handler_methods["_handle_do_nothing_action"].assert_not_called()
        mock_action_handler_methods["_handle_core_action_flow"].assert_not_called()
...
class TestActionHandlerFileSystem:
    ...
    def test_write_and_read_file(self, action_handler_instance: ActionHandler, fs: object) -> None:
