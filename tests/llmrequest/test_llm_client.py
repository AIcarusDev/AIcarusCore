...
class TestLLMClientRequestFlow:
    ...
    async def test_make_request_delegates_to_executor(self, client: LLMClient, mocker: MockerFixture):
    ...
    async def test_make_request_with_images_calls_media_processor(self, client: LLMClient, mocker: MockerFixture):
        ...
        mock_media_processor.assert_awaited_once_with(["fake_image_data"], None, "http://localhost:7890")
        
        call_args, call_kwargs = mock_executor_execute.call_args
        assert call_kwargs.get("processed_images") == [{"b64_data": "processed_data", "mime_type": "image/jpeg"}]