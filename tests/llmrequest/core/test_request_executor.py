...
from src.llmrequest.core.models import (APIKeyManager, NetworkError,
...
def mock_key_manager(mocker: MockerFixture) -> MagicMock:
    ...
    manager.get_available_keys.side_effect = [
        ["key1", "key2"], 
        ["key2"], 
        []
    ]
...
class TestRequestExecutor:
    ...
    async def test_successful_first_attempt(self, executor: RequestExecutor, mock_handler: MagicMock, mocker: MockerFixture):
        """测试：第一次尝试就成功。"""
        mock_api_call = mocker.patch.object(executor, "_make_api_call", new_callable=AsyncMock, return_value={"text": "Success"})
        
        result = await executor.execute_request(
            handler=mock_handler, model_name="test", request_type="chat", is_streaming=False, prompt="test",
            system_prompt=None, processed_images=None, generation_params={},
            tools=None, tool_choice=None, text_to_embed=None,
            max_retries=1, interruption_event=None, stream_chunk_delay=0.0, enable_google_search=False
        )
    ...
    async def test_switches_key_on_rate_limit(self, executor: RequestExecutor, mock_handler: MagicMock, mocker: MockerFixture):
        """测试：遇到 RateLimitError 后切换密钥并成功。"""
        mock_api_call = mocker.patch.object(
            executor, "_make_api_call",
    ...
    async def test_abandons_key_on_permission_denied(self, executor: RequestExecutor, mock_handler: MagicMock, mocker: MockerFixture):
    ...
    async def test_all_retries_fail_returns_error_dict(self, executor: RequestExecutor, mock_handler: MagicMock, mocker: MockerFixture):
        """测试：所有重试均失败后返回结构化错误。"""
