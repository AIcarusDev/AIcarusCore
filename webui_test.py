        ...
        for provider, expected_keys in expected_env_keys_map.items():
            for ek in expected_keys:
                if ek in missing_critical_keys_summary:
                    ...
                    if not is_opt or not any(
                        k_ollama in found_keys_in_env for k_ollama in expected_env_keys_map.get("OLLAMA", [])
                    ):
    ...
    if "new_ui_prompt_components" not in st.session_state:
        try:
            initial_output_format = (
                ThoughtPromptBuilder.PROMPT_TEMPLATE.split("严格以json字段输出：")[1]
                .split("请输出你的思考 JSON：")[0]
                .strip()  # <-- 修改
            )
        except IndexError:
...
def llm_configuration_sidebar() -> None:
    with st.sidebar:
        ...
        action_provider = st.selectbox(
            "提供商 ",
            ["GEMINI", "OPENAI", "CLAUDE", "GROQ", "OLLAMA"],
            key="action_prov_cfg_ui",
            index=0,  # 更改key防止冲突 (末尾加空格)
        )
        action_model_name = st.text_input(
            f"{action_provider} 模型名 ", "gemini-1.5-flash-latest", key="action_mod_cfg_ui"
        )
        ...
        summary_model_name = st.text_input(
            f"{summary_provider} 模型名  ", "gemini-1.5-flash-latest", key="summary_mod_cfg_ui"
        )
        ...
        intrusive_model_name = st.text_input(
            f"{intrusive_provider} 模型名   ", "gemini-1.5-flash-latest", key="intrusive_mod_cfg_ui"
        )
            ...
            if key_errors:
                ...
                try:
                    ...
                    for purpose_key, (prov_name, model_n) in model_configs_ui.items():
                        if prov_name and model_n and hasattr(_llm_models_cfg, purpose_key):
                            mp = ModelParams(
                                provider=prov_name, model_name=model_n, temperature=temp, max_output_tokens=max_tokens
                            )
                            setattr(_llm_models_cfg, purpose_key, mp)
                        elif prov_name and model_n and not hasattr(_llm_models_cfg, purpose_key):
                            logger.warning(
                                f"模型用途 '{purpose_key}' 在 AllModelPurposesConfig 中没有预定义属性，请检查 aicarus_configs.py。"
                            )
                    ...
                    temp_root_cfg_for_llm.intrusive_thoughts_module_settings.enabled = (
                    ...
                    async def initialize_llm_clients_async_ui() -> None:
        ...
        if not st.session_state.storage_initialized:
            db_host = st.text_input(
                "数据库地址", os.getenv("ARANGODB_HOST", "http://localhost:8529"), key="db_host_cfg_ui"
            )
            db_name = st.text_input(
                "数据库名", os.getenv("ARANGODB_DATABASE", "aicarus_core_ui_test"), key="db_name_cfg_ui"
            )
            ...
            if st.button("🔌 连接数据库", key="init_db_cfg_btn_ui"):
                ...
                async def connect_to_database_ui() -> None:
                    ...
                    logger.info(
                        f"WebUI: 数据库连接参数已通过环境变量设置: HOST={db_host}, DB={db_name}, USER={db_user}"
                    )
                    ...
                    try:
                        ...
                        if (
                            st.session_state.core_initializer.conn_manager
                            and st.session_state.core_initializer.conn_manager.db
                        ):
                            st.session_state.storage_initialized = True
                            st.success(
                                "数据库连接成功！🎉 (通过 CoreSystemInitializer 的新方法，并从环境变量读取UI输入)"
                            )
                            st.rerun()  #  替换 st.experimental_rerun()
                        else:
                            st.error(
                                "数据库初始化后，CoreSystemInitializer.conn_manager 或其内部db为空！😿"
                            )  # <-- 修改错误信息
                            st.session_state.storage_initialized = False
                    except Exception as e_db_init_ui:
                        st.error(
                            f"数据库连接失败 (通过 CoreSystemInitializer 的新方法): {e_db_init_ui} 😿"
                        )  # <-- 修改错误信息
...
async def show_original_ui() -> None:
    ...
    if not st.session_state.storage_initialized:
        ...
        return
    if (
        st.session_state.core_initializer is None
        or st.session_state.core_initializer.main_consciousness_llm_client is None
        or st.session_state.core_initializer.action_handler_instance is None
        or st.session_state.core_initializer.db_handler is None
        or st.session_state.core_initializer.root_cfg is None
    ):  # 确保 root_cfg 也已通过 Initializer 设置
    ...
    initial_state_orig = (
        AIStateManager.INITIAL_STATE
    )  # <-- 改用 AIStateManager 的初始状态，这里保持不变，因为它不是直接的配置读取
    ...
    if action_handler_instance and thought_storage_service_ui:  # <-- 修改依赖检查
        ...
        action_handler_instance.set_dependencies(  # <-- 修改依赖设置
            thought_service=thought_storage_service_ui, comm_layer=comm_layer_for_ui_test
        )
    ...
    with st.form("original_thought_form_ui"):
        _bot_name_orig = st.text_input(
            "机器人名称", persona_cfg_orig.bot_name, key="orig_bot_name_ui"
        )  # 哼，人家的名字才不是随便填的！
        mood_orig = st.text_input("当前心情", initial_state_orig["mood"], key="orig_mood_ui")
        previous_thinking_orig = st.text_area(
            "上一轮思考", initial_state_orig["previous_thinking"], height=100, key="orig_prev_think_ui"
        )
        thinking_guidance_orig = st.text_area(
            "思考方向指引", initial_state_orig["thinking_guidance"], height=100, key="orig_think_guidance_ui"
        )
        current_task_orig = st.text_input("当前任务", initial_state_orig["current_task"], key="orig_current_task_ui")
        action_result_info_orig = st.text_area(
            "上轮行动结果", initial_state_orig["action_result_info"], height=100, key="orig_action_result_ui"
        )
        pending_action_status_orig = st.text_input(
            "待处理行动状态", initial_state_orig["pending_action_status"], key="orig_pending_action_ui"
        )
        recent_context_orig = st.text_area(
            "最近上下文信息",
            initial_state_orig["recent_contextual_information"],
            height=150,
            key="orig_context_ui",
        )
    ...
    if (
        submitted_orig_thought
        and main_llm_client
        and event_storage_service_ui
        and thought_storage_service_ui
        and root_cfg
        and action_handler_instance
    ):  # <-- 修改依赖检查
        st.info("正在生成思考...请稍候。")
        try:
            ...
            temp_core_logic_for_prompt_ui = CoreLogicFlow(
                ...
                intrusive_generator_instance=getattr(
                    st.session_state.core_initializer, "intrusive_generator_instance", None
                ),
            ...
            with st.spinner("思考生成中..."):
                ...
                (
                    ...
                    full_prompt_text_sent,
                    system_prompt_sent,
                ) = await temp_core_logic_for_prompt_ui._generate_thought_from_llm(
                    ...
                    intrusive_thought_str=f"你突然有一个神奇的念头：{intrusive_thought_orig}"
                    if intrusive_thought_orig
                    else "",
                )
            ...
            if generated_thought_json:
                ...
                if generated_thought_json.get("action_to_take"):
                    ...
                    st.session_state.current_thought_context_for_next_step = generated_thought_json.get(
                        "think", "无特定思考上下文。"
                    )
                else:
            ...
            with st.expander("发送给主意识LLM的完整Prompt (原始UI)", expanded=False):
                st.text_area(
                    "System Prompt:",
                    value=st.session_state.get("last_system_prompt_sent_orig", ""),
                    height=150,
                    disabled=True,
                    key="orig_system_prompt_display_ui",
                )
                st.text_area(
                    "User Prompt:",
                    value=st.session_state.get("last_full_prompt_sent_orig", ""),
                    height=400,
                    disabled=True,
                    key="orig_full_prompt_display_ui",
                )
    ...
    if (
        "last_thought_json_orig" in st.session_state
        and st.session_state.last_thought_json_orig
        ...
        with st.form("action_decision_form_ui"):
            action_desc = st.text_input(
                "行动描述", st.session_state.get("action_description_for_next_step", ""), key="act_desc_ui"
            )
            action_motive = st.text_input(
                "行动动机", st.session_state.get("action_motivation_for_next_step", ""), key="act_motive_ui"
            )
            current_thought_context_act = st.text_area(
                "当前思考上下文",
                st.session_state.get("current_thought_context_for_next_step", ""),
                height=100,
                key="act_thought_context_ui",
            )
        ...
        if submitted_action_decision:
            if (
                action_handler_instance
                and action_handler_instance.action_llm_client
                and hasattr(action_handler_instance, "ACTION_DECISION_PROMPT_TEMPLATE")
                and hasattr(action_handler_instance, "AVAILABLE_TOOLS_SCHEMA_FOR_GEMINI")
            ):  # 确保 action_llm_client 和模板/schema 属性也存在 #
                st.info("正在进行动作决策...")
                try:
                    tools_json_str_ui = json.dumps(
                        action_handler_instance.AVAILABLE_TOOLS_SCHEMA_FOR_GEMINI, indent=2, ensure_ascii=False
                    )
                    ...
                    with st.spinner("决策LLM正在努力思考调用哪个工具..."):
                        ...
                        decision_response = (
                            await action_handler_instance.action_llm_client.make_llm_request(  # 使用 make_llm_request #
                    ...
                    if decision_response.get("error"):
                        ...
                        elif decision_response.get(
                            "text"
                        ):  # Fallback if tool_calls not directly in response but in text #
...
async def show_new_ui() -> None:
    ...
    _persona_cfg_new_ui = config.persona
    ...
    with tabs[7]:
        comps["current_time_block"] = st.text_input(
            "Current Time (用于 System Prompt)",
            value=comps["current_time_block"],
            key="new_time_ui",
            help="传递给模型的当前时间字符串，会包含在 System Prompt 中。",  # 更改标签和帮助文本 #
        )
    ...
    if st.button("🧠 生成思考 (新版UI)", type="primary", key="new_generate_thought_btn_ui"):
        if st.session_state.core_initializer and st.session_state.core_initializer.main_consciousness_llm_client:
            ...
            final_system_prompt_for_llm_ui = (
                f"当前时间：{comps['current_time_block']}\n你是{bot_name_val_new_ui}；\n{comps['persona_block']}"
            )
            ...
            previous_thinking_for_user_prompt = AIStateManager.INITIAL_STATE[
                "previous_thinking"
            ]  # 假设这些也是从历史记录中来 # <-- 修改
            ...
            final_user_prompt_for_llm = user_prompt_template_with_custom_schema.format(
                ...
                else "",  # 这是UI中的 "Intrusive Thought" #
            )

            with st.expander("发送给主意识LLM的完整Prompt (新版UI组合结果)", expanded=False):
                st.text_area(
                    "System Prompt:",
                    value=final_system_prompt_for_llm_ui,
                    height=150,
                    disabled=True,
                    key="new_final_system_prompt_display_ui",
                )
                st.text_area(
                    "User Prompt:",
                    value=final_user_prompt_for_llm,
                    height=400,
                    disabled=True,
                    key="new_final_user_prompt_display_ui",
                )

            with st.spinner("新版UI的LLM也在努力思考中...喵..."):
                try:
                    response_data_new_ui = (
                        await st.session_state.core_initializer.main_consciousness_llm_client.make_llm_request(
