...
try:
    from src.config.alcarus_configs import (
        AlcarusRootConfig, PersonaSettings, LLMClientSettings, ModelParams, ProxySettings,
        CoreLogicSettings, IntrusiveThoughtsSettings, ProvidersConfig, ProviderSettings, ProviderModels,
        DatabaseSettings, LoggingSettings, InnerConfig,
...
def load_custom_env(dotenv_path: str = ".env", override: bool = True) -> tuple[bool, int, list[str]]:
    ...
    expected_env_keys_map = {
        "GEMINI": ["GEMINI_KEY"], "OPENAI": ["OPENAI_KEY", "OPENAI_BASE_URL"],
        "CLAUDE": ["ANTHROPIC_KEY"], "GROQ": ["GROQ_KEY"],
        "OLLAMA": ["OLLAMA_BASE_URL", "OLLAMA_BASE_URL"]
    }
    all_expected_keys_flat = set(k for keys in expected_env_keys_map.values() for k in keys)
    found_keys_in_env = set()
    try:
        with open(dotenv_path, encoding="utf-8") as f: lines = f.readlines()
        i = 0
        while i < len(lines):
            line = lines[i].strip(); i += 1
            if not line or line.startswith("#"): continue
            if "=" not in line: logger.warning(f".env 行 {i} 格式无效: {line}"); continue
            key, value_part = line.split("=", 1); key = key.strip(); value_part = value_part.strip()
            final_value = value_part; open_quote_char = None
            if value_part.startswith("'") or value_part.startswith('"'):
                open_quote_char = value_part[0]
                if len(value_part) > 1 and value_part.endswith(open_quote_char): final_value = value_part[1:-1]
                elif open_quote_char:
                    accumulated_value_lines = [value_part[1:]]; found_closing_quote = False
                    while i < len(lines):
                        next_line_raw = lines[i].rstrip("\n"); i += 1
                        if next_line_raw.endswith(open_quote_char) and not next_line_raw.endswith(f"\\{open_quote_char}"):
                            accumulated_value_lines.append(next_line_raw[:-1]); found_closing_quote = True; break
                        else: accumulated_value_lines.append(next_line_raw)
                    full_multiline_value = "\n".join(accumulated_value_lines)
                    if found_closing_quote: final_value = full_multiline_value
                    else: logger.warning(f"多行值 {key} 未找到结束引号"); final_value = value_part
            try: final_value = bytes(final_value, "utf-8").decode("unicode_escape")
            except UnicodeDecodeError: logger.debug(f"unicode_escape 解码失败: {final_value[:30]}...")
            if key and (override or key not in os.environ):
                os.environ[key] = final_value; logger.debug(f"加载 env: {key}"); loaded_count += 1
            if key in all_expected_keys_flat: found_keys_in_env.add(key)
        ...
        return True, loaded_count, sorted(list(set(detailed_missing_summary)))
...
def initialize_session_state():
    if 'llm_initialized' not in st.session_state: st.session_state.llm_initialized = False
    if 'root_cfg_minimal' not in st.session_state: st.session_state.root_cfg_minimal = None
    if 'main_llm_client' not in st.session_state: st.session_state.main_llm_client = None
    if 'action_llm_client' not in st.session_state: st.session_state.action_llm_client = None
    if 'action_handler_instance' not in st.session_state: st.session_state.action_handler_instance = None
    if 'env_load_attempted_this_session' not in st.session_state:
        ...
        st.session_state.env_missing_keys_info = load_custom_env()
    ...
    if 'new_ui_prompt_components' not in st.session_state:
        try:
            initial_output_format = CoreLogic.PROMPT_TEMPLATE.split("严格以json字段输出：")[1].split("请输出你的思考 JSON：")[0].strip()
...
{CoreLogic.INITIAL_STATE["recent_contextual_information"]}""",
            ...
            "thinking_guidance_block": CoreLogic.INITIAL_STATE["thinking_guidance"].split("：",1)[-1].strip(), # 取冒号后的内容
    ...
    with st.sidebar:
        ...
        if st.session_state.env_loaded_successfully:
            ...
            if st.session_state.env_missing_keys_info:
                ...
                for item in st.session_state.env_missing_keys_info: st.caption(f"- {item}")
            # else: st.info("常用环境变量在 .env 中均找到。") #太啰嗦了，去掉
        else:
            st.error("未能从 .env 加载环境变量！请检查或直接设置系统环境变量。")

        st.subheader("主意识LLM")
        main_provider = st.selectbox("提供商", ["GEMINI", "OPENAI", "CLAUDE", "GROQ", "OLLAMA"], key="main_prov_cfg", index=0)
        main_model = st.text_input(f"{main_provider} 模型名", "gemini-1.5-flash-latest", key="main_mod_cfg")
        
        st.subheader("动作决策LLM")
        action_provider = st.selectbox("提供商", ["GEMINI", "OPENAI", "CLAUDE", "GROQ", "OLLAMA"], key="action_prov_cfg", index=0)
        ...
        if st.button("✔️ 应用配置并初始化LLM", key="init_llm_cfg_btn"):
            ...
            if action_provider != "OLLAMA" and not os.getenv(prov_map.get(action_provider)):
                key_errors.append(f"动作决策LLM ({action_provider}) 的 {prov_map.get(action_provider)} 未在环境变量找到！")

            if key_errors:
                for msg in key_errors: st.error(msg)
            else:
                try:
                    def create_client(p, m, t, mt):
                        return ProcessorClient(**{k:v for k,v in {"model": {"provider": p, "name": m}, "temperature": t, "maxOutputTokens": mt}.items() if v is not None})
                    ...
                    _persona_s = PersonaSettings(bot_name="配置小喵", description="专门测试配置的喵", profile="喜欢检查环境变量")
                    ...
                    # 主意识
                    _main_mp = ModelParams(provider=main_provider, model_name=main_model, temperature=temp, max_output_tokens=max_tokens)
                    _main_pa = main_provider.lower()
                    if hasattr(_providers_s, _main_pa):
                        _prov_set_main = getattr(_providers_s, _main_pa)
                        if _prov_set_main is None: _prov_set_main = ProviderSettings(models=ProviderModels())
                        if _prov_set_main.models is None: _prov_set_main.models = ProviderModels()
                        _prov_set_main.models.main_consciousness = _main_mp
                        setattr(_providers_s, _main_pa, _prov_set_main)
                    
                    # 动作决策
                    _action_mp = ModelParams(provider=action_provider, model_name=action_model, temperature=temp, max_output_tokens=max_tokens)
                    _action_pa = action_provider.lower()
                    if hasattr(_providers_s, _action_pa):
                        _prov_set_action = getattr(_providers_s, _action_pa)
                        if _prov_set_action is None: _prov_set_action = ProviderSettings(models=ProviderModels())
                        if _prov_set_action.models is None: _prov_set_action.models = ProviderModels()
                    ...
                    st.session_state.root_cfg_minimal = AlcarusRootConfig(
                        inner=_inner_s, llm_client_settings=_llm_client_s, persona=_persona_s, proxy=_proxy_s,
                        core_logic_settings=_core_logic_s, intrusive_thoughts_module_settings=_intrusive_s,
                        providers=_providers_s, database=_db_s, logging=_log_s
                    )
...
def show_original_ui():
    ...
    with st.form("original_thought_form"):
        bot_name_orig = st.text_input("机器人名称", persona_cfg_orig.bot_name, key="orig_bot_name")
        # ... 其他输入框 ...
        mood_orig = st.text_input("当前心情", initial_state_orig["mood"], key="orig_mood")
        recent_context_orig = st.text_area("最近上下文", initial_state_orig["recent_contextual_information"], height=150, key="orig_context")
...
def show_new_ui():
    ...
    tab_titles = ["👤人格面具", "📜任务规则", "💬上下文历史", "⚙️思考指引", "😊心情", "⚡️侵入思维", "📐输出格式", "⏱️当前时间"]
    tabs = st.tabs(tab_titles)

    with tabs[0]:
        comps["persona_block"] = st.text_area("Persona / System Prompt", value=comps["persona_block"], height=200, key="new_persona",
                                             help="定义AI的角色、背景、性格、说话风格等。")
    with tabs[1]:
        comps["task_rules_block"] = st.text_area("Task & Rules", value=comps["task_rules_block"], height=200, key="new_task",
                                                help="明确AI当前需要完成的具体任务和必须遵守的规则。")
    with tabs[2]:
        comps["context_history_block"] = st.text_area("Context & History", value=comps["context_history_block"], height=300, key="new_context",
                                                     help="提供相关的对话历史、之前的思考、行动结果等上下文信息。")
    with tabs[3]:
        comps["thinking_guidance_block"] = st.text_area("Thinking Guidance", value=comps["thinking_guidance_block"], height=100, key="new_guidance",
                                                       help="引导AI接下来的思考方向。")
    with tabs[4]:
        comps["mood_block"] = st.text_input("Mood", value=comps["mood_block"], key="new_mood", help="AI当前的心情状态。")
    with tabs[5]:
        comps["intrusive_thought_block"] = st.text_input("Intrusive Thought", value=comps["intrusive_thought_block"], key="new_intrusive",
                                                        help="一个突然产生的、可能不相关的念头。")
    with tabs[6]:
        comps["output_format_block"] = st.text_area("Output Format Requirement (JSON Schema)", value=comps["output_format_block"], height=250, key="new_output_format",
                                                   help="严格定义模型输出的JSON结构。")
    with tabs[7]:
        comps["current_time_block"] = st.text_input("Current Time", value=comps["current_time_block"], key="new_time",
                                                    help="传递给模型的当前时间字符串。")
    ...
    if st.button("🧠 生成思考 (新版UI)", type="primary", key="new_generate_thought_btn"):
        if st.session_state.main_llm_client:
            ...
            try:
                ...
                modified_template_for_new_ui = part1_before_json_schema + \
            ...
            final_prompt_for_llm = modified_template_for_new_ui.format(
                ...
                intrusive_thought=f"你突然有一个神奇的念头：{comps['intrusive_thought_block']}" if comps['intrusive_thought_block'] else ""
            )
            ...
            with st.spinner("新版UI的LLM也在努力思考中...喵..."):
                try:
                    ...
                    if response_data_new.get("error"):
                        ...
                        if json_to_parse_new.startswith("```json"): json_to_parse_new = json_to_parse_new[7:-3].strip()
                        elif json_to_parse_new.startswith("```"): json_to_parse_new = json_to_parse_new[3:-3].strip()
                        try:
        ...
        if 'last_raw_response_new' in st.session_state:
            with st.expander("主意识LLM的原始回复 (新版UI)", expanded=False):
                st.text_area("", value=st.session_state.last_raw_response_new, height=200, disabled=True, key="new_raw_output_display")
...
def main_app():
    ...
    # 页面导航
    page_options = {
        "原始版本测试器 (旧)": show_original_ui,
        "仿截图版本测试器 (新)": show_new_ui
    }
