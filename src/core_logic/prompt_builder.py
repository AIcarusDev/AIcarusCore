...
class ThoughtPromptBuilder:
    ...
    async def build_prompts_components(
        ...
        if current_level == "core" and "focus" in final_ctrl_schema_props:
            all_platform_ids = [pid for pid in platform_builder_registry.get_all_builders() if pid != "core"]
            if all_platform_ids:
        ...
        response_schema = {"type": "object", "properties": {"internal_state": {"type": "object", "properties": {"mood": {"type": "string"}, "think": {"type": "string"}, "goal": {"type": "string"}}, "required": ["mood", "think", "goal"]}, "consciousness_control": {"type": "object", "properties": final_ctrl_schema_props, "maxProperties": 1}, "action": {"type": "object", "properties": final_act_schema_props}}, "required": ["internal_state"]}
        
        (external_info_block, meta_info_block, history_components, processed_raw_events) = await self._get_external_and_meta_info_blocks(
        ...
        system_prompt_blocks = {
            "aicarus_rule_block": AICARUS_RULE, "current_time": get_formatted_time_for_llm(),
            "persona_block": self._get_persona_block(), "available_platforms_block": await self._get_available_platforms_block(),
            "current_state_block": await self._get_current_state_block(current_level, current_platform_id, current_conv_id),
            "behavior_guidelines_block": self._get_behavior_guidelines_block(current_level),
            "internal_info_block": internal_info_block, "input_XML_block_description": self._get_input_xml_block_description(current_level),
            "available_consciousness_controls": self._get_controls_descriptions(current_level, builder, core_builder),
            "available_actions": self._get_actions_descriptions(current_level, builder, core_builder),
        }
        
        user_prompt_blocks = {
            "action_response_block": action_response_block, "meta_info_block": meta_info_block,
            "external_info_block": external_info_block
        }

        prompt_components_obj = PromptComponents(
            system_prompt_blocks=system_prompt_blocks, user_prompt_blocks=user_prompt_blocks,
    ...
    async def get_last_valid_text_message(self, conversation_id: str) -> str | None:
        ...
        prompt_components, _ = await format_chat_history_for_llm(
            event_storage=self.event_storage, conversation_id=session.conversation_id,
            bot_id=session.bot_id, platform=session.platform, bot_profile=await session.get_bot_profile(),
            conversation_type=session.conversation_type, conversation_name=session.conversation_name,
            last_processed_timestamp=session.last_processed_timestamp, is_first_turn=False,
        )
    ...
    async def _get_available_platforms_block(self) -> str:
        # (此函数逻辑不变)
        if not self.core_ws_server: return "平台通信服务尚未准备就绪。"
    ...
    async def _get_current_state_block(self, level: str, platform_id: str, conv_id: str | None) -> str:
        # (此函数逻辑不变)
        if not self.chat_session_manager: return "会话管理器尚未准备就绪。"
        if level == "core": return "你当前专注于：发呆/自我思考。"
        elif level == "platform": return f"你当前专注于：{platform_id} 平台。"
            ...
            else: return f"你当前正在 qq 上与{session.conversation_name or '对方'}私聊"
        return "未知状态"
    ...
    def _get_controls_descriptions(self, level, builder, core_builder) -> str:
        ...
        plat_desc = builder.get_level_consciousness_controls_descriptions(level) if level != "core" and builder else ""
    ...
    def _get_actions_descriptions(self, level, builder, core_builder) -> str:
        ...
        plat_desc = builder.get_level_actions_descriptions(level) if level != "core" and builder else ""
    ...
    async def _get_external_and_meta_info_blocks(
        ...
        if not self.chat_session_manager: return "会话管理器尚未准备就绪。", "", None, None
        if level == "core":
            external_info = await self.unread_info_service.get_platform_summary()
        elif level == "platform":
            external_info = await self.unread_info_service.get_conversation_list_summary(platform_id)
        elif level == "cellular" and conv_id:
            session = self.chat_session_manager.sessions.get(conv_id)
            if not session: return "错误：找不到会话档案。", "", None, None
            bot_profile = await session.get_bot_profile()
            history_components, processed_raw_events = await format_chat_history_for_llm(
                event_storage=self.event_storage, conversation_id=session.conversation_id,
                bot_id=session.bot_id, platform=session.platform, bot_profile=bot_profile,
                conversation_type=session.conversation_type, conversation_name=session.conversation_name,
                last_processed_timestamp=session.last_processed_timestamp, is_first_turn=self.is_context_switch_flag,
            )
            if history_components.conversation_name:
                session.conversation_name = history_components.conversation_name
            unread_summary_str = await self.unread_info_service.generate_unread_summary_text(exclude_conversation_id=session.conversation_id)
            external_info = (f"<Conversation_Info>\n{history_components.conversation_info_block}\n</Conversation_Info>\n\n"
