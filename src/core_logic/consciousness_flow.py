...
class CoreLogic:
    ...
    async def _gather_context(self) -> tuple[str, str, list[str]]:
        ...
        initial_empty_context_info: str = self.state_manager.INITIAL_STATE[
        ...
        try:
            ...
            system_lifecycle_events_raw: list[dict[str, Any]] = (
                await self.event_storage_service.get_recent_chat_message_documents(
                    duration_minutes=chat_history_duration_minutes,
            ...
            if hasattr(self.core_comm_layer, "adapter_clients_info") and isinstance(
                self.core_comm_layer.adapter_clients_info, dict
            ):
    ...
    async def _process_and_store_thought(self, thought_json: dict, prompts: dict, context: dict) -> str | None:
        """
        处理并存储思考结果，烦人的数据整理活。
        """
    ...
    async def _dispatch_action(self, thought_json: dict, saved_thought_key: str, recent_context: str) -> None:
    ...
    async def _reply_to_master(
    ...
    ) -> None:  # 增加了 current_thought_key 参数
        ...
        if self.action_handler_instance:
            if not current_thought_key:
                ...
                self.logger.warning(
                    "在 _reply_to_master 中，current_thought_key 为 None，将使用虚拟key 'direct_reply_action'。这可能导致思考文档状态更新不准确。"
                )
                ...
                if not current_thought_key:  # 再次检查，理论上不应发生
                    ...
                    if not send_success:
                        self.logger.error(
                            f"向主人UI (adapter_id: {master_adapter_id}) 发送回复失败了（Fallback模式）。"
                        )
                    return
    ...
    async def _core_thinking_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            if generated_thought:
                ...
                saved_key = (
                    await self._process_and_store_thought(  # 将这行移到 reply_to_master 和 _dispatch_action 之前
                        generated_thought,
