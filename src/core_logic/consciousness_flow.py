...
class CoreLogic:
    ...
    def trigger_immediate_thought_cycle(self, handover_summary: str | None = None, last_focus_think: str | None = None) -> None:
        ...
        if handover_summary or last_focus_think:
            if hasattr(self.state_manager, "set_next_handover_info") and callable(getattr(self.state_manager, "set_next_handover_info")):
                ...
                self.logger.error("AIStateManager 对象没有 set_next_handover_info 方法或该方法不可调用，交接信息可能丢失！")
    ...
    async def _reply_to_master(
        self, content_str: str, current_thought_key: str | None
    ) -> None:
        ...
        if self.action_handler_instance:
            if not current_thought_key:
                ...
                return
            self.logger.info(f"通过 ActionHandler 发送对主人的回复。Action ID: {reply_action_id}, 关联思考Key: {current_thought_key}")
    ...
    async def _core_thinking_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            if hasattr(self.chat_session_manager, 'is_any_session_active') and self.chat_session_manager.is_any_session_active():
            ...
            master_chat_str, other_context_str, image_list = await self.context_builder.gather_context_for_core_thought()
            ...
            if hasattr(self.prompt_builder.unread_info_service, 'get_structured_unread_conversations'):
                try:
                    structured_unread_conversations = await self.prompt_builder.unread_info_service.get_structured_unread_conversations()
                    ...
                    self.logger.error(f"调用 get_structured_unread_conversations 失败: {e_struct_unread}", exc_info=True)
            else:
                self.logger.warning("UnreadInfoService (via prompt_builder) 缺少 get_structured_unread_conversations 方法。")
            ...
            if action_id_to_mark_as_seen and self.state_manager.thought_service:
                ...
                marked_seen = await self.state_manager.thought_service.mark_action_result_as_seen(action_id_to_mark_as_seen)
                if marked_seen:
            ...
            if (
                ...
                random_thought_doc = await self.state_manager.thought_service.get_random_unused_intrusive_thought_document()
            ...
            user_prompt = await self.prompt_builder.build_user_prompt(current_state, master_chat_str, intrusive_thought_str)
            ...
            if generated_thought:
                ...
                if action_desc_from_llm and action_desc_from_llm.lower() != "null":
                    ...
                    if not current_action_id:  # 包括 None 或空字符串 ""
                        ...
                        new_action_id = str(uuid.uuid4())
                        self.logger.warning(f"LLM意图行动 '{action_desc_from_llm}' 但提供的 action_id '{current_action_id}' 无效。重新生成: {new_action_id}")
                        generated_thought["action_id"] = new_action_id


                saved_key = await self.thought_persistor.store_thought(generated_thought, prompts_for_storage, context_for_storage)
                ...
                if focus_conversation_id and isinstance(focus_conversation_id, str):
                        ...
                        if target_conv_details:
                            ...
                            if platform and conv_type:
                                try:
                                    ...
                                    self.logger.info(f"已调用 chat_session_manager.activate_session_by_id 针对会话 {focus_conversation_id} (Platform: {platform}, Type: {conv_type})")
                                except Exception as e_activate:
                                    self.logger.error(f"调用 chat_session_manager.activate_session_by_id 失败: {e_activate}", exc_info=True)
                            else:
                                self.logger.error(f"无法从结构化未读信息中找到会话 {focus_conversation_id} 的 platform 或 type，无法激活。")
                        else:
                            self.logger.error(f"LLM决策激活的会话ID {focus_conversation_id} 未在当前的结构化未读列表中找到，无法激活。")
                    else:
                    ...
                    self.logger.warning(f"LLM返回的 active_focus_on_conversation_id 不是有效的字符串ID: {focus_conversation_id} (类型: {type(focus_conversation_id)})。忽略激活请求。")
