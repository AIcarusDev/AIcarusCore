...
class CoreSystemInitializer:
    ...
    def __init__(self):
        ...
        self.action_log_service: Optional[ActionLogStorageService] = None # 新增 ActionLog 服务
    ...
    async def _initialize_llm_clients(self) -> None:
        ...
        def _create_client_from_model_params(model_params_cfg, purpose_key_for_log: str) -> Optional[ProcessorClient]:
            try:
                ...
                if not actual_provider_name_str or not actual_model_api_name:
                    logger.error(f"配置错误：用途为 '{purpose_key_for_log}' 的模型 未明确指定 'provider' 或 'model_name' 字段。")
                    return None
                model_for_client_constructor: Dict[str, str] = {"provider": actual_provider_name_str.upper(), "name": actual_model_api_name}
                ...
                logger.info(f"为用途 '{purpose_key_for_log}' 成功创建 ProcessorClient 实例 (模型: {client_instance.llm_client.model_name}, 提供商: {client_instance.llm_client.provider}).")
                return client_instance
            except Exception as e:
                logger.error(f"为用途 '{purpose_key_for_log}' (提供商 '{model_params_cfg.provider if model_params_cfg else '未知'}') 创建LLM客户端时发生未知错误: {e}。", exc_info=True)
                return None
        try:
            ...
            for purpose_key, client_attr_name in model_purpose_map.items():
                ...
                if model_params_cfg and hasattr(model_params_cfg, 'provider') and hasattr(model_params_cfg, 'model_name'):
                    if purpose_key == "intrusive_thoughts" and (not config.intrusive_thoughts_module_settings or not config.intrusive_thoughts_module_settings.enabled):
                    ...
                    if not client_instance:
                        if purpose_key == "main_consciousness": raise RuntimeError(f"核心组件 '{purpose_key}' 的LLM客户端初始化失败。")
                        else: logger.warning(f"可选组件 '{purpose_key}' 的LLM客户端未能初始化。相关功能可能受限。")
                elif model_params_cfg is None:
                     logger.info(f"在 [llm_models] 中未找到用途为 '{purpose_key}' 的模型配置，跳过其客户端创建。")
                     setattr(self, client_attr_name, None)
                else:
                    logger.error(f"配置错误：用途为 '{purpose_key}' 的模型配置类型不正确 (期望有效的模型参数配置)，得到 {type(model_params_cfg)}。将跳过此客户端的创建。")
                    setattr(self, client_attr_name, None)
    ...
    async def _initialize_database_and_services(self) -> None:
        ...
        if hasattr(self.event_storage_service, 'initialize_infrastructure') and callable(self.event_storage_service.initialize_infrastructure):
        ...
        if hasattr(self.conversation_storage_service, 'initialize_infrastructure') and callable(self.conversation_storage_service.initialize_infrastructure):
        ...
        if hasattr(self.thought_storage_service, 'initialize_infrastructure') and callable(self.thought_storage_service.initialize_infrastructure):
        ...
        if hasattr(self.action_log_service, 'initialize_infrastructure') and callable(self.action_log_service.initialize_infrastructure):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if not self.thought_storage_service or not self.event_storage_service or not self.action_log_service: # 检查新增的 action_log_service
                missing_deps_msg = []
                ...
                logger.critical(f"核心存储服务 ({', '.join(missing_deps_msg)}) 未初始化，无法正确设置 ActionHandler 依赖。小猫咪要闹情绪了！")
            ...
            if self.message_processor and hasattr(self.message_processor, 'process_event') and callable(self.message_processor.process_event):
            ...
            logger.info(f"核心 WebSocket 通信层 (CoreWebsocketServer) 准备在 ws://{ws_host}:{ws_port} 上监听，并已关联 ActionHandler。")
    ...
    async def start(self) -> None:
        ...
        try:
            if self.intrusive_generator_instance and config.intrusive_thoughts_module_settings.enabled:
                ...
                if self.intrusive_thread: logger.info("侵入性思维后台生成线程已启动。")
                else: logger.warning("侵入性思维后台生成线程未能启动 (可能由于其内部依赖未满足或模块已在配置中禁用)。")
            ...
            for task in done:
                ...
                if task.cancelled(): logger.info(f"任务 '{task_name}' 被取消。")
                elif task.exception(): 
                    exc = task.exception()
                    logger.critical(f"关键任务 '{task_name}' 因未捕获的异常而意外终止: {exc!r}", exc_info=exc)
                    if exc: raise exc 
                else: logger.info(f"任务 '{task_name}' 已正常结束。")
            for task in pending:
                ...
                if not task.done(): 
                    task.cancel() 
                    try: await task 
                    except asyncio.CancelledError: logger.info(f"挂起的任务 '{task_name}' 已成功响应取消请求并结束。")
                    except Exception as e_cancel: logger.error(f"尝试取消挂起任务 '{task_name}' 时发生意外错误: {e_cancel}", exc_info=True)
        except asyncio.CancelledError: logger.info("AIcarus Core 主启动流程 (start 方法) 被外部取消。")
        except Exception as e: 
    ...
    async def shutdown(self) -> None:
        ...
        if self.intrusive_thread is not None and self.intrusive_thread.is_alive():
            ...
            if self.intrusive_thread.is_alive(): logger.warning("警告：侵入性思维后台线程在10秒超时后仍未结束。可能需要强制处理或检查其循环逻辑。")
            else: logger.info("侵入性思维后台生成线程已成功结束。")
        if self.core_comm_layer:
        ...
        llm_clients_to_close: List[Optional[ProcessorClient]] = [ 
            self.main_consciousness_llm_client, self.action_llm_client, self.summary_llm_client,
            self.intrusive_thoughts_llm_client, self.embedding_llm_client
        ]
        for llm_client_wrapper in llm_clients_to_close:
            if llm_client_wrapper and hasattr(llm_client_wrapper.llm_client, '_close_session_if_any') and callable(llm_client_wrapper.llm_client._close_session_if_any): 
                try:
                    logger.info(f"尝试关闭 LLM 客户端 ({llm_client_wrapper.llm_client.provider} - {llm_client_wrapper.llm_client.model_name}) 的底层 aiohttp 会话 (如果存在)...")
                    await llm_client_wrapper.llm_client._close_session_if_any() # type: ignore
                except Exception as e_llm_close: logger.warning(f"关闭 LLM 客户端的底层会话时出错: {e_llm_close}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.critical(f"AIcarus Core (main.py __main__): 顶层执行过程中发生未捕获的严重异常: {main_execution_exc}", exc_info=True)
    finally:
