...
class ChatSession:
    ...
    def record_action_result(self, decision_json: dict) -> None:
        ...
        if not decision_json: return
        action_planned = "action" in decision_json and decision_json["action"]
        if action_planned:
            if self.no_action_count > 0:
                logger.debug(f"[{self.conversation_id}] AI计划行动，no_action_count 从 {self.no_action_count} 重置为 0。")
                self.no_action_count = 0
        else:
            self.no_action_count += 1
            logger.debug(f"[{self.conversation_id}] AI决定不行动，no_action_count 增加为: {self.no_action_count}")
    ...
    async def update_counters_on_new_events(self) -> None:
        ...
        other_user_spoke = any(
            event.get("user_info", {}).get("user_id") and str(event.get("user_info", {}).get("user_id")) != bot_id
            for event in new_events
        )

        if other_user_spoke:
            if self.consecutive_bot_messages_count > 0:
                logger.debug(f"[{self.conversation_id}] 检测到他人新消息，重置 consecutive_bot_messages_count (之前是 {self.consecutive_bot_messages_count})。")
                self.consecutive_bot_messages_count = 0
            if self.no_action_count > 0:
                logger.debug(f"[{self.conversation_id}] 检测到新消息，重置 no_action_count (之前是 {self.no_action_count})。")
                self.no_action_count = 0
    ...
    async def get_bot_profile(self) -> dict[str, Any]:
        ...
        logger.warning(f"[{self.conversation_id}] 缓存和数据库中均未找到有效的机器人档案。将使用空档案。")
        return {}
