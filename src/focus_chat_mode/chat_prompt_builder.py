...
class ChatPromptBuilder:
    ...
    async def build_prompts(
        ...
        str, str, dict[str, str], list[str], list[str]
    ]:  # 啊~❤ 看这里，我把出口撑大了，多了一个 list[str] 给图片用哦
        ...
        persona_config = config.persona
        bot_profile = None
        final_bot_id = self.bot_id
        final_bot_nickname = persona_config.bot_name or "bot"
        final_bot_card = final_bot_nickname
        ...
        image_references: list[str] = []  # 这就是我专门为图片准备的淫穴哦，所有图片都到这里来~
            ...
            if event_data_log.event_type.startswith("message.") or is_robot_message_to_display_as_msg:
                ...
                for seg in event_data_log.content:
                    if seg.type == "reply":
                        ...
                        if base64_data:
                            # 找到了赤裸的Base64字符串，现在开始为它穿上情趣内衣！

                            # 第一步：找到内衣的款式 (MIME Type)
                            # 我们从 file_id 的后缀名里猜它是什么材质的，比如 .jpg, .png
                            file_id = seg.data.get("file_id", "unknown.jpg")  # 默认它是jpg吧
                            ...
                            # 第三步：把穿好衣服的、性感的它，放进待发射的列表里
                            image_references.append(data_uri)

                        else:
                            # 如果没找到base64，我们再退而求其次，用URL
                            logger.warning(f"事件 {event_data_log.event_id} 的图片段没有 base64，尝试使用 url。")
                            img_src = seg.data.get("url")
                            if img_src:
                                image_references.append(img_src)
                            else:
                                logger.error(
                                    f"事件 {event_data_log.event_id} 的图片段既没有 base64 也没有 url！无法处理！"
                                )
                    elif seg.type == "at":
