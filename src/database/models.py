...
from aicarus_protocols import Seg as ProtocolSeg # Seg 通常是 content 列表的元素类型
...
class AttentionProfile:
    ...
    relevant_topic_tags: List[str] = field(default_factory=list) # AI为此会话标注的相关话题标签，用于基于内容的注意力加权。
    last_ai_interaction_timestamp: Optional[int] = None  # AI上次与此会话进行有效互动的时间戳 (毫秒, UTC)。
    last_significant_event_timestamp: Optional[int] = None  # 此会话中上次发生对AI而言“重要事件”（如被@）的时间戳 (毫秒, UTC)。
    cooldown_until_timestamp: Optional[int] = None      # 如果AI暂时将此会话置于“冷却”或“低优先级”状态，此字段记录该状态解除的时间戳 (毫秒, UTC)。
    is_suspended_by_ai: bool = False                    # 标记此会话是否被AI主动置于“暂停处理”或“忽略”的状态。
    suspension_reason: Optional[str] = None             # 如果被暂停，记录暂停的原因。
    ai_custom_notes: Optional[str] = None               # AI针对此会话记录的内部自定义备注或策略提示。
    # 可以考虑加入更多交互统计指标，例如：
    ...
    @classmethod
    def from_dict(cls, data: Optional[Dict[str, Any]]) -> "AttentionProfile":
        """
...
class EnrichedConversationInfo:
    ...
    def from_protocol_and_event_context(
        cls,
        proto_conv_info: Optional[ProtocolConversationInfo], # 来自协议的原始 ConversationInfo 对象
    ...
    @classmethod
    def from_db_document(cls, doc: Optional[Dict[str, Any]]) -> Optional["EnrichedConversationInfo"]:
...
class DBEventDocument:
    ...
    user_info: Optional[Dict[str, Any]] = None # 发起事件的用户信息 (UserInfo对象的字典表示)
    conversation_info: Optional[Dict[str, Any]] = None # 事件发生的会话信息 (ConversationInfo对象的字典表示)
    raw_data: Optional[Dict[str, Any]] = None    # 来自适配器的原始事件数据，用于调试或特殊处理
    protocol_version: str = "1.4.0" # 事件数据所遵循的协议版本号

    # 为便于数据库查询而从 user_info 和 conversation_info 中提取的关键ID
    user_id_extracted: Optional[str] = None         # 提取出的用户ID
    conversation_id_extracted: Optional[str] = None # 提取出的会话ID
    ...
    def from_protocol(cls, proto_event: ProtocolEvent) -> "DBEventDocument":
        ...
        if not isinstance(proto_event, ProtocolEvent): # 基本的类型检查
            logger.error(f"传递给 DBEventDocument.from_protocol 的对象类型错误，期望 ProtocolEvent，得到 {type(proto_event)}")
        ...
        if proto_event.user_info: # proto_event.user_info 是 ProtocolUserInfo 类型
            if hasattr(proto_event.user_info, 'to_dict') and callable(proto_event.user_info.to_dict):
                ...
                user_info_dict = proto_event.user_info
                 ...
                 logger.warning(f"事件 '{proto_event.event_id}' 的 user_info 无法转换为字典: {type(proto_event.user_info)}。")
        ...
        if proto_event.conversation_info: # proto_event.conversation_info 是 ProtocolConversationInfo 类型
            if hasattr(proto_event.conversation_info, 'to_dict') and callable(proto_event.conversation_info.to_dict):
                ...
                logger.warning(f"事件 '{proto_event.event_id}' 的 conversation_info 无法转换为字典: {type(proto_event.conversation_info)}。")
...
class ActionRecordDocument:
    ...
    # 关于动作目标的信息
    target_conversation_id: Optional[str] = None # 目标会话ID
    target_user_id: Optional[str] = None         # 目标用户ID
    
    parameters: Optional[Dict[str, Any]] = None # 执行此动作所需的具体参数
    result_data: Optional[Dict[str, Any]] = None  # 动作成功执行后返回的数据（如果有）
    error_message: Optional[str] = None         # 动作执行失败时的错误信息
    
    # 可选的关联信息，用于追踪和调试
    initiated_by_event_id: Optional[str] = None # （如果适用）触发此动作的原始事件的ID
    initiated_by_thought_id: Optional[str] = None # （如果适用）触发此动作的AI思考过程的ID
    completed_at_timestamp: Optional[int] = None # 动作完成（成功或失败）的时间戳 (毫秒, UTC)
    ...
    @classmethod
    def from_dict(cls, data: Optional[Dict[str, Any]]) -> Optional["ActionRecordDocument"]:
