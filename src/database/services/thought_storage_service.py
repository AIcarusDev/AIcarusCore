...
class ThoughtStorageService:
    ...
    async def update_action_status_in_thought_document(
        ...
        try:
            # --- 性感小猫咪的诊断探针 Start ---
            self.logger.debug(
                f"主人，进入 update_action_status_in_thought_document 方法，doc_key: {doc_key}, action_id: {action_id}"
            )
            ...
            if hasattr(collection, "update"):
                ...
                if not callable(collection.update):
                    self.logger.error(
                        f"  啊哦！collection.update (类型: {type(collection.update)}) 不是一个可调用的方法！它肯定被什么骚东西附身了！"
                    )
            else:
                self.logger.error(
                    f"  致命错误！获取到的 collection (类型: {type(collection)}) 没有 'update' 这个性感方法！"
                )
            # --- 性感小猫咪的进一步诊断探针 End ---
            ...
            await asyncio.to_thread(
                collection.update, {"_key": doc_key, **patch_document_for_db}
            )  # 使用 collection.update
            self.logger.info(
                f"思考文档 '{doc_key}' 中动作 '{action_id}' 的状态已被小猫咪成功更新为: {status_update_dict}，爽！"
            )
            return True
        except DocumentUpdateError as e:
            self.logger.error(
                f"更新思考文档 '{doc_key}' 中动作 '{action_id}' 的状态时，数据库高潮失败: {e}", exc_info=True
            )
            return False
        except TypeError as te:  # 专门捕获 TypeError，看看是不是我们怀疑的那个
            self.logger.error(
                f"啊！更新思考文档 '{doc_key}' 中动作 '{action_id}' 状态时捕获到 TypeError: {te}！就是这个小坏蛋！",
                exc_info=True,
            )
            return False
        except Exception as e:
            self.logger.error(
                f"更新思考文档 '{doc_key}' 中动作 '{action_id}' 状态时发生意外的痉挛（错误）: {e}", exc_info=True
            )
            return False
    ...
    async def mark_action_result_as_seen(self, action_id_to_mark: str) -> bool:
        ...
        try:
            ...
            if action_attempted_current.get("result_seen_by_shimo") is True:
                self.logger.info(
                    f"动作ID '{action_id_to_mark}' (文档key: {doc_key_to_update}) 的结果已经被标记为已阅，无需重复操作。"
                )
                return True
            ...
            self.logger.info(
                f"已成功将文档 '{doc_key_to_update}' (action_id: {action_id_to_mark}) 的 action_attempted.result_seen_by_shimo 标记为 true。"
            )
            return True
        except DocumentUpdateError as e_update:
            self.logger.error(
                f"更新文档 '{doc_key_to_update}' (action_id: {action_id_to_mark}) 标记已阅时数据库操作失败: {e_update}",
                exc_info=True,
            )
            return False
        except Exception as e_general:
            self.logger.error(
                f"更新文档 '{doc_key_to_update}' (action_id: {action_id_to_mark}) 标记已阅时发生意外错误: {e_general}",
                exc_info=True,
            )
            return False
