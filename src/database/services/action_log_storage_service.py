...
class ActionLogStorageService:
    ...
    async def save_action_attempt(
        ...
        original_event_id: Optional[str] = None, # 触发此动作的原始事件ID (如用户消息)
        ...
        try:
            ...
            if existing_doc:
                self.logger.info(f"ActionLog 中已存在 action_id '{action_id}' 的记录。当前状态: {existing_doc.get('status')}")
    ...
    async def update_action_log_with_response(
        ...
        try:
            ...
            if result and result.get('_id'):
                self.logger.info(f"ActionLog 中动作 '{action_id}' 的状态已更新为 '{status}'。更新字段: {final_doc_to_update}")
                return True
            else:
                self.logger.warning(f"尝试更新 ActionLog 中动作 '{action_id}' 失败，可能记录不存在或更新未生效。Update result: {result}")
                existing_doc = await asyncio.to_thread(collection.get, action_id)
                if not existing_doc:
