...
import asyncio
import os
import time # 保留 time 模块的导入
from typing import Any, List, Optional, Tuple, Dict # 确保 Dict 被导入
...
class ArangoDBConnectionManager:
    ...
    def __init__(self, client: ArangoClient, db: StandardDatabase, core_collection_configs: Dict[str, List[Tuple[List[str], bool, bool]]]):
        """
        初始化连接管理器。
        ...
        self.logger = logger # 使用类属性存储日志记录器
        self.logger.info(f"ArangoDBConnectionManager 已使用数据库 '{db.name}' 初始化。")

    @classmethod
    async def create_from_config(cls,
                                 database_config_obj: Any, # 通常是一个包含数据库连接信息的配置对象 (如 dataclass)
        ...
        if not all([host, database_name]): # host 和 database_name 是必需的
            missing_params = []
            if not host: missing_params.append("host (或 ARANGODB_HOST 环境变量)")
            if not database_name: missing_params.append("database_name (或 ARANGODB_DATABASE 环境变量)")
    ...
    async def ensure_core_infrastructure(self) -> None:
        ...
        self.logger.info("正在确保核心数据库基础设施 (集合和特定索引) 已按配置就绪...")
        if not self.core_collection_configs: # 如果没有提供核心集合配置，则记录警告并跳过
            self.logger.warning("未提供核心集合配置 (core_collection_configs)，跳过基础设施保障步骤。")
            return
        ...
        self.logger.info("核心数据库基础设施已保障。")

    async def ensure_collection_with_indexes(
        self,
        collection_name: str,
        index_definitions: Optional[List[Tuple[List[str], bool, bool]]] = None # 索引定义: (字段列表, 是否唯一, 是否稀疏)
    ) -> StandardCollection:
        """
        ...
        if not has_collection: # 如果集合不存在
            ...
            try:
                ...
                self.logger.error(f"创建集合 '{collection_name}' 失败: {e}", exc_info=True)
    ...
    async def _apply_indexes_to_collection(
        ...
        try:
                ...
                if field_key_str in existing_indexes_by_fields_str:
                    ...
                    if recreate_if_different and \
                       (existing_idx.get('unique', False) != unique or existing_idx.get('sparse', False) != sparse):
                        ...
                        self.logger.info(f"索引 {fields} 在 '{collection_name}' 上已存在但定义不同。正在尝试重建。")
                        try:
                            await asyncio.to_thread(collection.delete_index, existing_idx['id'])
                            self.logger.info(f"已删除旧索引 {existing_idx['id']}，准备创建新索引。")
                        except Exception as e_del_idx:
                            self.logger.warning(f"无法删除现有索引 {existing_idx['id']} 以进行重建: {e_del_idx}。将跳过此索引的重建。")
                            continue # 如果删除失败，则跳过此索引的重建，继续处理下一个
                    else:
                        # 如果索引已存在且不需要重建（或定义一致），则跳过创建
                        self.logger.debug(f"索引 {fields} 在 '{collection_name}' 上已存在或定义匹配。跳过创建。")
                        continue

                # 如果索引不存在或已被删除以待重建，则创建新索引
                self.logger.debug(f"正在为集合 '{collection_name}' 应用索引: 字段={fields}, 唯一={unique}, 稀疏={sparse}")
            ...
            self.logger.warning(f"无法为集合 '{collection_name}' 完全确保所有索引。错误: {e}。尝试的索引定义: {indexes_to_create}")
        except Exception as e: # 捕获其他在应用索引过程中发生的意外错误
            self.logger.error(f"为集合 '{collection_name}' 应用索引时发生意外错误: {e}", exc_info=True)
    ...
    async def execute_query(self, query: str, bind_vars: Optional[Dict[str, Any]] = None) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            # self.logger.debug(f"正在执行AQL (前100字符): {query[:100]}{'...' if len(query) > 100 else ''}") # 日志过于频繁时可注释掉
            # AQL的 execute 方法是同步的，因此用 to_thread 包装
            cursor = await asyncio.to_thread(self.db.aql.execute, query, bind_vars=final_bind_vars, count=False) # count=False表示不获取总匹配数，提高性能
            results = await asyncio.to_thread(list, cursor) # 将游标结果转换为列表
            return results
        except AQLQueryExecuteError as e: # ArangoDB 特定的查询执行错误
            self.logger.error(f"AQL查询执行失败。错误详情: {e.errors()}") # e.errors() 提供更详细的错误信息
            self.logger.error(f"失败的AQL查询: {query}")
            if bind_vars: self.logger.error(f"失败的AQL绑定参数: {bind_vars}")
            return None # 查询失败时返回 None
        except Exception as e: # 捕获其他所有可能的意外错误
            self.logger.error(f"AQL查询执行期间发生意外错误: {e}", exc_info=True)
            return None # 查询失败时返回 None
...
class CoreDBCollections:
    ...
    INDEX_DEFINITIONS: Dict[str, List[Tuple[List[str], bool, bool]]] = {
        ...
        THOUGHTS: [
            ...
            (["action_attempted.action_id"], True, True), # 假设action_id在存在时是唯一的，且action_attempted对象本身可能不存在 (稀疏)
        ],
        ACTION_LOGS: [
        ...
        CONVERSATIONS: [
            ...
            (["attention_profile.is_suspended_by_ai"], False, True), # 筛选被AI暂停处理的会话 (稀疏，因为该字段可能不存在或为false)
