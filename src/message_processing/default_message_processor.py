...
from aicarus_protocols import Seg, SegBuilder, UserInfo, ConversationInfo as ProtocolConversationInfo, ConversationType
...
class DefaultMessageProcessor:
    ...
    async def process_event(
        ...
        if not isinstance(proto_event, ProtocolEvent): # 基本类型检查
            self.logger.error(f"process_event 收到的 event 不是预期的 ProtocolEvent 类型，而是 {type(proto_event)}。已跳过处理。")
            return

        self.logger.debug(f"开始处理事件: {proto_event.event_type}, ID: {proto_event.event_id}, Platform: {proto_event.platform}, BotID: {proto_event.bot_id}")
        
        if proto_event.user_info:
            self.logger.debug(f"  UserInfo: id={proto_event.user_info.user_id}, nickname={proto_event.user_info.user_nickname}")
        if proto_event.conversation_info:
            self.logger.debug(f"  ConversationInfo: id={proto_event.conversation_info.conversation_id}, type={proto_event.conversation_info.type}, name={proto_event.conversation_info.name}")
        ...
        try:
            ...
            if proto_event.conversation_info and proto_event.conversation_info.conversation_id:
                ...
                upsert_result_key = await self.conversation_service.upsert_conversation_document(conversation_doc_to_upsert)
    ...
    def _extract_text_from_protocol_event_content(self, content_seg_list: Optional[List[Seg]]) -> str:
        """
        ...
        try:
            ...
            if text_content == "完整测试":
                ...
                if not original_sender_info or not original_sender_info.user_id:
                    self.logger.error("无法获取原始发送者信息 (UserInfo 或 user_id 为空)，无法执行 '完整测试' 的回复动作。")
                    return True
                ...
                # 为动作事件准备目标会话信息
                action_target_conversation_info: Optional[ProtocolConversationInfo] = None
                ...
                if original_conversation_info and \
                   original_conversation_info.type == ConversationType.GROUP and \
                   original_conversation_info.conversation_id:
    ...
    async def _handle_request_event(self, proto_event: ProtocolEvent, websocket: WebSocketServerProtocol) -> None:
        ...
        try:
            ...
            if proto_event.event_type == "request.friend.add":
                ...
                if not proto_event.content or \
                   not isinstance(proto_event.content[0], Seg) or \
                   not proto_event.content[0].data:
                ...
                action_user_info_for_approve: Optional[UserInfo] = None
                if proto_event.user_info and proto_event.user_info.user_id:
                     action_user_info_for_approve = UserInfo(
                         user_id=proto_event.user_info.user_id,
                         user_nickname=proto_event.user_info.user_nickname
                     ) # 只传递必要信息
    ...
    async def _handle_action_response_event(self, proto_event: ProtocolEvent, websocket: WebSocketServerProtocol) -> None:
        """处理来自适配器的动作响应事件。"""
        try:
            # 动作响应事件的内容通常包含原始动作的ID、执行状态、结果数据或错误信息
            # 假设这些信息在 proto_event.content[0].data 中
            original_action_id_from_response = "未知原始动作ID"
            if proto_event.content and \
               isinstance(proto_event.content[0], Seg) and \
               proto_event.content[0].data:
                action_response_data: dict = proto_event.content[0].data
                original_action_id_from_response = action_response_data.get("original_event_id", original_action_id_from_response)
                ...
                self.logger.info(
                    f"收到对动作 '{original_action_id_from_response}' 的响应: "
                    f"状态码={status_code}, 消息='{message}'"
                )
                if data_payload:
                ...
                self.logger.info(f"动作响应 '{original_action_id_from_response}' 的后续处理逻辑 (如更新思考文档) 待实现。")


            else:
                self.logger.warning(f"收到的动作响应事件 {proto_event.event_id} (类型: {proto_event.event_type}) 内容格式不正确或为空。")
