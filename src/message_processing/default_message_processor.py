...
from aicarus_protocols import MessageBase, Seg, BaseMessageInfo, UserInfo, GroupInfo 
...
class DefaultMessageProcessor:
    ...
    async def process_message(self, message: MessageBase, websocket: WebSocketServerProtocol) -> None:
        ...
        if message.message_info.interaction_purpose == "platform_meta" or \
           message.message_info.interaction_purpose == "platform_heartbeat":
        ...
        if message.message_segment: 
            ...
            if current_outer_segment and isinstance(current_outer_segment, Seg): # 确保 current_outer_segment 是有效的 Seg 对象
            ...
            logger.debug(f"准备处理的 segments_to_process 列表 (共 {len(segments_to_process)} 个段): {[s.to_dict() for s in segments_to_process]}")

            for seg_item in segments_to_process: 
                logger.debug(f"循环内检查 seg_item: type='{seg_item.type}', data_type='{type(seg_item.data)}', data_value='{str(seg_item.data)[:100]}'") # 截断 data_value 以防过长
                if seg_item.type == "text":
                    if isinstance(seg_item.data, str):
                        # 去除前后空格后追加
                        text_content += seg_item.data.strip()
                        logger.debug(f"已追加文本 (str): '{seg_item.data.strip()}'. 当前 text_content: '{text_content}'")
                    # 兼容旧的/错误的 data 格式，即文本段的 data 是一个包含 "text" 键的字典
                    elif isinstance(seg_item.data, dict) and "text" in seg_item.data: 
                        logger.warning(f"文本段的 data 是一个字典 (协议要求为字符串)，尝试使用 data['text']。段: {seg_item.to_dict()}")
                        text_content += str(seg_item.data.get("text", "")).strip()
                        logger.debug(f"已追加文本 (from dict): '{str(seg_item.data.get('text', '')).strip()}'. 当前 text_content: '{text_content}'")
                    else:
                        logger.warning(f"文本段的 data 既不是字符串也不是预期的字典格式。类型: {type(seg_item.data)}, 内容: {str(seg_item.data)[:100]}")
                else:
        ...
        if text_content.strip().lower() == "测试test":
            ...
            poke_action_data: dict[str, Any] = {
                "target_user_id": original_sender_user_id
            }
            if original_group_id: 
                ...
                for seg_obj_item in current_outer_segment_for_save.data: 
                    if isinstance(seg_obj_item, Seg): # 期望是 Seg 对象
                        ...
                        try:
                            ...
                            content_segments_to_save.append({"type": "error", "data": f"原始段数据: {str(seg_obj_item)}"})
                    # ... (其他类型的错误处理可以添加) ...
