...
class PlatformBuilderRegistry:
    ...
    def discover_and_register_builders(self, package) -> None:
        """
                    ...
                    if issubclass(item, BasePlatformBuilder) and item is not BasePlatformBuilder:
                        try:
                            ...
                            if platform_id in self._builders:
                                logger.warning(f"发现重复的翻译官！平台'{platform_id}'的翻译官被'{item.__name__}'覆盖了！")
    ...
    def get_builder(self, platform_id: str) -> Optional[BasePlatformBuilder]:
        """根据平台ID，找一个翻译官出来干活。"""
