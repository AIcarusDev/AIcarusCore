...
import time
import uuid
from typing import Any, List, Dict, Optional, Tuple, Callable, TYPE_CHECKING
...
from aicarus_protocols import Event as ProtocolEvent # 引入协议事件
...
class ActionHandler:
    ...
    def __init__(self) -> None:
        ...
        self.core_communication_layer: Optional['CoreWebsocketServer'] = None
        ...
        self.action_log_service: Optional[ActionLogStorageService] = None # 新增 ActionLog 服务依赖
        self.thought_trigger: Optional[asyncio.Event] = None
        self._pending_actions: Dict[str, Tuple[asyncio.Event, Optional[str], str, Dict[str, Any]]] = {} # thought_doc_key can be None
        self.logger.info(f"{self.__class__.__name__} instance created.")

    def set_dependencies(
        self,
        thought_service: Optional[ThoughtStorageService] = None,
        event_service: Optional[EventStorageService] = None,
        action_log_service: Optional[ActionLogStorageService] = None, # 新增注入
        ...
        self.logger.info("ActionHandler 的依赖已成功设置 (thought_service, event_service, action_log_service, comm_layer)。")

    def set_thought_trigger(self, trigger_event: Optional[asyncio.Event]) -> None:
    ...
    def _create_llm_client_from_config(self, purpose_key: str) -> Optional[ProcessorClient]:
        try:
            ...
            if not model_params_cfg or not hasattr(model_params_cfg, 'provider'):
                self.logger.error(f"配置错误：在 AlcarusRootConfig.llm_models 下未找到模型用途键 '{purpose_key}' 对应的有效模型配置，或类型不匹配。")
                return None
            ...
            if not actual_provider_name_str or not actual_model_name_str:
                self.logger.error(f"配置错误：模型 '{purpose_key}' (提供商: {actual_provider_name_str or '未知'}) 未指定 'provider' 或 'model_name'。")
                return None
            ...
            if final_proxy_port_str and final_proxy_port_str.isdigit(): final_proxy_port = int(final_proxy_port_str)
            if final_proxy_host and final_proxy_port: self.logger.info(f"ActionHandler LLM客户端将尝试使用环境变量中的代理: {final_proxy_host}:{final_proxy_port}")
            else: self.logger.info("ActionHandler LLM客户端未在环境变量中检测到完整代理配置，将不使用代理。")
            ...
            processor_constructor_args = {"model": model_for_client_constructor, "proxy_host": final_proxy_host, "proxy_port": final_proxy_port, **vars(general_llm_settings_obj), **model_specific_kwargs}
        ...
        except AttributeError as e_attr: self.logger.error(f"配置访问错误 (AttributeError) 创建LLM客户端 (用途: {purpose_key}) 时: {e_attr}", exc_info=True); return None
        except Exception as e: self.logger.error(f"创建LLM客户端 (用途: {purpose_key}) 时发生未知错误: {e}", exc_info=True); return None

    async def initialize_llm_clients(self) -> None:
        if self.action_llm_client and self.summary_llm_client: return
        ...
        if not self.action_llm_client:
            ...
            if not self.action_llm_client: self.logger.critical("行动决策LLM客户端初始化失败。"); raise RuntimeError("行动决策LLM客户端初始化失败。")
        if not self.summary_llm_client:
            ...
            if not self.summary_llm_client: self.logger.critical("信息总结LLM客户端初始化失败。"); raise RuntimeError("信息总结LLM客户端初始化失败。")
        self.logger.info("行动处理模块的LLM客户端按需初始化完成。")

    async def _summarize_tool_result_async(self, original_query: str, original_motivation: str, tool_output: Any) -> str:
        if not self.summary_llm_client: self.logger.error("信息总结LLM客户端未初始化，无法进行信息总结。"); return f"错误：信息总结功能当前不可用。原始工具输出: {str(tool_output)[:200]}..."
        self.logger.info(f"正在调用LLM对工具结果进行信息总结。原始意图: '{original_query[:50]}...'")
        try: raw_tool_output_str = json.dumps(tool_output, indent=2, ensure_ascii=False)
        except TypeError: raw_tool_output_str = str(tool_output)
        summary_prompt = INFORMATION_SUMMARY_PROMPT_TEMPLATE.format(original_query_or_action=original_query, original_motivation=original_motivation, raw_tool_output=raw_tool_output_str)
        response = await self.summary_llm_client.make_llm_request(prompt=summary_prompt, is_stream=False)
        if response.get("error"): error_message = f"总结信息时LLM调用失败: {response.get('message', '未知API错误')}"; self.logger.error(error_message); return error_message
        summary_text = response.get("text")
        if summary_text is None or not summary_text.strip(): self.logger.warning("信息总结LLM调用成功，但未返回有效的文本内容。"); return "未能从工具结果中总结出有效信息。"
        self.logger.info(f"信息总结完成。摘要 (前100字符): {summary_text[:100]}..."); return summary_text.strip()

    async def _handle_action_timeout(self, action_id: str, thought_doc_key: Optional[str], original_action_description: str, original_action_to_send: Dict[str, Any]) -> None:
        if action_id in self._pending_actions:
            self.logger.warning(f"动作 '{action_id}' ({original_action_description}) 超时未收到响应！")
            pending_event, stored_thought_doc_key, stored_original_action_description, _ = self._pending_actions.pop(action_id)
            pending_event.set()
            ...
            if self.action_log_service:
                await self.action_log_service.update_action_log_with_response(action_id=action_id, status="timeout", response_timestamp=timeout_timestamp, error_info="Action response timed out")
            else:
            ...
            if not is_direct_reply_action_for_timeout and stored_thought_doc_key and self.thought_storage_service:
                ...
                await self.thought_storage_service.update_action_status_in_thought_document(stored_thought_doc_key, action_id, update_payload_thought)
            elif not is_direct_reply_action_for_timeout and not stored_thought_doc_key:
                 self.logger.warning(f"动作 '{action_id}' ({stored_original_action_description}) 超时，但没有关联的 thought_doc_key，无法更新思考文档。")
    ...
    async def handle_action_response(self, response_event_data: Dict[str, Any]) -> None:
        ...
        if not original_action_id: self.logger.error("收到的 action_response 事件缺少 'event_id' (原始 action_id)，无法处理！"); return

        if original_action_id not in self._pending_actions:
            self.logger.warning(f"收到一个未知的或已处理/超时的 action_response，对应的 action_id: {original_action_id}。可能已经超时或重复响应。")
            return

        pending_event, stored_thought_doc_key, stored_original_action_description, original_action_sent_dict = self._pending_actions.pop(original_action_id)
        pending_event.set()
        ...
        result_details_from_response: Optional[Dict[str, Any]] = None
        ...
        if content_list and isinstance(content_list, list) and len(content_list) > 0:
            ...
            if isinstance(first_segment, dict) and first_segment.get("type") == "action_status":
                ...
                if response_status_str == "success":
                    ...
                    if result_details_from_response: final_result_for_thought += f" 详情: {json.dumps(result_details_from_response, ensure_ascii=False)}"
                else:
                    action_successful = False
            ...
            else: self.logger.warning(f"Action_response for '{original_action_id}' 的 content[0] 不是预期的 'action_status' 类型。Content: {first_segment}")
        else: self.logger.warning(f"Action_response for '{original_action_id}' 缺少有效的 content 列表。")

        if self.action_log_service:
            await self.action_log_service.update_action_log_with_response(action_id=original_action_id, status=response_status_str, response_timestamp=response_timestamp, response_time_ms=response_time_ms, error_info=error_message_from_response if not action_successful else None, result_details=result_details_from_response)
        ...
        if not is_direct_reply_action_for_response and stored_thought_doc_key and self.thought_storage_service:
            ...
            self.logger.info(f"已更新思考文档 '{stored_thought_doc_key}' 中动作 '{original_action_id}' 的状态为: {'成功' if action_successful else '失败'}。")
            ...
            self.logger.info(f"直接回复动作 '{original_action_id}' 收到响应，仅更新ActionLog。")
             ...
             self.logger.info(f"动作 '{original_action_id}' ({stored_original_action_description}) 收到响应，但没有关联的 thought_doc_key，跳过更新思考文档。")
        ...
        if action_successful and self.event_storage_service:
            ...
            self.logger.info(f"成功的平台动作 '{original_action_id}' (类型: {event_to_save_in_events.get('event_type')}) 已作为事件存入 events 表。")


    async def _execute_platform_action(
        self,
        action_to_send: Dict[str, Any],
        thought_doc_key: Optional[str], # Made Optional for direct replies
        original_action_description: str
    ) -> Tuple[bool, str]:
        self.logger.debug(f"进入 _execute_platform_action。action_log_service 是否为 None: {self.action_log_service is None}")

        if not self.core_communication_layer: self.logger.error("CoreWebsocketServer 未设置!"); return False, "内部错误：通信层不可用。"
        if not self.action_log_service: self.logger.error("ActionLogService 未设置!"); return False, "内部错误：动作日志服务不可用。"
        ...
        if not is_direct_reply_action and not thought_doc_key:
            self.logger.error(f"严重错误：尝试执行非直接回复动作 '{original_action_description}' 但缺少 thought_doc_key。")
        ...
        if not is_direct_reply_action and (not self.thought_storage_service or not self.event_storage_service):
            missing = []
            if not self.thought_storage_service: missing.append("ThoughtStorageService")
            if not self.event_storage_service: missing.append("EventStorageService")
            m_str = ', '.join(missing)
        ...
        if not core_action_id: core_action_id = str(uuid.uuid4()); action_to_send["event_id"] = core_action_id
        ...
        conversation_id_for_action = action_to_send.get("conversation_info", {}).get("conversation_id", "unknown_conv_id")
        ...
        self.logger.info(f"准备发送平台动作 '{core_action_id}' ({original_action_description}) 到适配器 '{target_adapter_id}' (原始平台类型: '{platform_id_from_event}')...")
        ...
        if not is_direct_reply_action and self.thought_storage_service and thought_doc_key:
            await self.thought_storage_service.update_action_status_in_thought_document(
                thought_doc_key, core_action_id, {"status": "EXECUTING_AWAITING_RESPONSE", "sent_to_adapter_at": current_timestamp_ms}
            )
        ...
        save_log_success = await self.action_log_service.save_action_attempt(action_id=core_action_id, action_type=action_type, timestamp=current_timestamp_ms, platform=platform_id_from_event, bot_id=bot_id_for_action, conversation_id=conversation_id_for_action, content=content_for_action)
        self.logger.info(f"action_log_service.save_action_attempt 调用完成。是否成功保存: {save_log_success}")
        if not save_log_success: self.logger.error(f"严重错误：未能将动作 '{core_action_id}' 的初始尝试记录到 ActionLog！")

        try:
            # 使用 target_adapter_id 进行发送
            send_to_adapter_successful = await self.core_communication_layer.send_action_to_adapter_by_id(adapter_id=target_adapter_id, action_event=action_to_send)
            ...
            self.logger.error(f"发送平台动作 '{core_action_id}' 到适配器 '{target_adapter_id}' 时发生意外异常: {e_send}", exc_info=True)
            error_timestamp = int(time.time() * 1000)
        ...
        self._pending_actions[core_action_id] = (response_received_event, thought_doc_key, original_action_description, action_to_send)
        ...
        try:
            ...
            if final_log_entry and final_log_entry.get("status") == "success": return True, f"动作 '{original_action_description}' 已成功执行并收到响应。"
            elif final_log_entry: return False, f"动作 '{original_action_description}' 执行失败: {final_log_entry.get('error_info', '未知错误')}"
    ...
    async def process_action_flow(
        ...
        try: await self.initialize_llm_clients()
        ...
        decision_prompt_text = ACTION_DECISION_PROMPT_TEMPLATE.format(current_thought_context=current_thought_context, action_description=action_description, action_motivation=action_motivation, relevant_adapter_messages_context=relevant_adapter_messages_context)
        if not self.action_llm_client:
        ...
        if decision_response.get("error"):
            ...
            if not llm_raw_output_text_for_decision:
                ...
                try:
                    ...
                    if json_string_to_parse.startswith("```json"): json_string_to_parse = json_string_to_parse[7:-3].strip()
                    elif json_string_to_parse.startswith("```"): json_string_to_parse = json_string_to_parse[3:-3].strip()
