...
class PendingActionManager:
    ...
    def __init__(
        ...
        self._pending_actions: dict[str, tuple[asyncio.Future, str | None, str, dict[str, Any], str | None]] = {}
    ...
    async def add_and_wait_for_action(
        ...
        self._pending_actions[action_id] = (response_future, thought_doc_key, original_action_description, action_to_send, motivation)
        try:
    ...
    async def handle_response(self, response_event_data: dict[str, Any]) -> None:
        ...
        pending_future, thought_doc_key, description, sent_dict, motivation = self._pending_actions.pop(original_action_id)
        ...
        if successful and original_action_type and original_action_type.endswith(".send_message"):
            ...
            if conversation_info and isinstance(conversation_info, dict):
                ...
                if conv_id and self.action_handler.chat_session_manager:
                    ...
                    if session:
                        logger.info(f"检测到 send_message 动作的回声，正在为动作 '{original_action_id}' 调用 session.signal_echo_received()！")
        ...
        tasks_to_gather = [
            self.action_log_service.update_action_log_with_response(
                action_id=original_action_id, status=status, response_timestamp=response_timestamp,
                response_time_ms=response_time_ms, error_info=None if successful else error_msg, result_details=details,
            )
        ]
        if thought_doc_key:
            _final_result_message = self._create_final_result_message(description, successful, error_msg, details)
            tasks_to_gather.append(
                self.thought_storage_service.save_action_result_to_thought(thought_key=thought_doc_key, result_text=_final_result_message)
            )
        if successful:
            tasks_to_gather.append(
                self._save_successful_action_as_event(original_action_id, sent_dict, response_event_data, motivation=motivation)
            )
        if tasks_to_gather:
    ...
    def _parse_response_content(self, data: dict[str, Any]) -> tuple[bool, str, str, dict | None]:
        content = data.get("content", [])
        if not content: return False, "unknown", "响应内容为空", None
        segment = content[0]
        ...
        if isinstance(segment, dict) and seg_type.startswith("action_response."):
            ...
            if status == "success": return True, "success", "", details
            else: return False, status, response_data.get("message", "适配器报告未知错误"), details
    ...
    def _create_final_result_message(self, desc: str, succ: bool, err: str, det: dict | None) -> str:
        if succ:
            msg = f"动作 '{desc}' 已成功执行。"
            if det: msg += f" 详情: {json.dumps(det, ensure_ascii=False)}"
            return msg
        return f"动作 '{desc}' 执行失败: {err}"

    async def _save_successful_action_as_event(self, action_id: str, sent_dict: dict[str, Any], resp_data: dict[str, Any], motivation: str | None = None) -> None:
        ...
        event_to_save["content"] = [{"type": "message_metadata", "data": {"message_id": message_id}}] + event_to_save.get("content", [])
        real_user_info = None
        if conv_info and isinstance(conv_info, dict):
            conv_id = conv_info.get("conversation_id")
            if conv_id and self.action_handler.chat_session_manager and (session := self.action_handler.chat_session_manager.sessions.get(str(conv_id))):
                bot_profile = await session.get_bot_profile()
                real_user_info = {"platform": session.platform, "user_id": bot_profile.get("user_id"), "user_nickname": bot_profile.get("nickname"), "user_cardname": bot_profile.get("card"), "role": bot_profile.get("role")}
        event_to_save["user_info"] = real_user_info or {"platform": resp_data.get("platform", "unknown_platform"), "user_id": resp_data.get("bot_id", "unknown_user_id"), "user_nickname": "AIcarus (Self)"}
    ...
    async def _get_sent_message_id_safe(self, event_data: dict[str, Any]) -> str:
        ...
        if not isinstance(event_data, dict): return default_id
            ...
            if isinstance(first_item, dict):
                ...
                if isinstance(response_data, dict):
                    ...
                    if isinstance(details_data, dict):
                        ...
                        if sent_message_id is not None: return str(sent_message_id)
        return default_id