...
class ActionSender:
    ...
    async def broadcast_action_to_adapters(self, action_event: dict[str, Any]) -> bool:
        ...
        try:
            ...
            results = await asyncio.gather(
                *(ws.send(action_json) for ws in self.connected_adapters.values()),
            ...
            logger.info(
                f"动作广播完成: {success_count}/{len(self.connected_adapters)} 个适配器尝试发送"
            )
    ...
    async def send_action_to_specific_adapter(
        ...
        display_name = self.adapter_clients_info.get(adapter_id, {}).get(
            "display_name", adapter_id or "未知"
        )
        ...
        try:
            ...
            logger.error(
                f"序列化单个动作事件为 JSON 时出错 (目标: '{display_name}'): {e_json}",
                exc_info=True,
            )
            return False
        try:
    ...
    async def send_action_to_adapter_by_id(
        self, adapter_id: str, action_event: dict[str, Any]
    ) -> bool:
        """通过适配器ID向其发送一个动作."""
