# 文件路径: src/os/apps/qq/qq_chat_session_manager.py
...
class QQChatSessionManager:
    ...
    async def get_or_create_session(self, conversation_entity_uid: str) -> QQChatSession | None:
        ...
        async with self.lock:
            ...
            # 步骤 2: 调用带有重试机制的健壮查询方法
            conv_entity_doc = await self.entity_graph_service.get_entity_by_key_with_retry(
                conversation_entity_uid
            )

            # 步骤 3: 如果成功获取到实体，则继续创建 Session
            ...
            # 步骤 4: 如果带重试的查询最终还是失败了
            else:
                logger.error(
                    f"无法为 '{conversation_entity_uid}' 创建会话，实体不存在或类型错误。"
                )
                return None
