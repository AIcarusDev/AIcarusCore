...
from .models import (APIKeyManager, APIResponseError, GenerationParams,
                     LLMClientError, NetworkError, PayloadTooLargeError,
...
class RequestExecutor:
    ...
    def __init__(self, key_manager: APIKeyManager, base_url: str, proxy_url: str | None):
    ...
    async def execute_request(
        ...
        for attempt_pass in range(max_retries + 1):
            if interruption_event and interruption_event.is_set():
                return {"error": False, "interrupted": True, "message": "Task interrupted before API call."}
            ...
            logger.info(f"开始第 {attempt_pass + 1}/{max_retries + 1} 次请求尝试轮。本轮可用密钥数: {len(available_keys)}")

            for key_idx, current_key in enumerate(available_keys):
                try:
                    path, params, headers, payload = handler.prepare_request_data(
                        model_name, request_type, is_streaming, prompt, system_prompt, 
                        images_for_request, generation_params, tools, tool_choice, 
                        text_to_embed, enable_google_search
                    )
                    
                    result = await self._make_api_call(
                        handler, current_key, path, params, headers, payload, is_streaming, 
                        request_type, stream_chunk_delay
                    )
                    return result
                    ...
                    logger.warning(f"尝试密钥 ...{current_key[-4:]} 失败: {type(e).__name__} - {e!s}")
                    last_exception = e
                except Exception as e:
                    logger.error(f"尝试密钥 ...{current_key[-4:]} 时发生未知严重错误: {e!s}", exc_info=True)
                    last_exception = e

            if attempt_pass < max_retries:
                wait_duration = INITIAL_RETRY_PASS_DELAY_SECONDS * (2**attempt_pass)
                logger.warning(f"第 {attempt_pass + 1} 次请求尝试轮未成功。等待 {wait_duration:.2f} 秒后重试。")
            ...
            try:
                async with session.post(
                    ...
                    if 200 <= status_code < 300:
                        if is_streaming:
                            return await handler.handle_streaming_response(http_response, stream_chunk_delay)
                        else:
                        ...
                        if status_code == 413: raise PayloadTooLargeError("请求体过大", status_code, response_text)
                        if status_code in [401, 403]: raise PermissionDeniedError("权限被拒绝", status_code, response_text, key_identifier=api_key)
                        if status_code == 429: raise RateLimitError("速率限制", status_code, response_text, key_identifier=api_key)
