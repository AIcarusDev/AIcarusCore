...
class ServiceBuilder:
    async def build_container(self) -> ServiceContainer:
        ...
        core_comm_layer = CoreWebsocketServer(
            host=config.server.host, port=config.server.port, event_receiver=event_receiver,
            action_sender=action_sender, event_storage_service=db_services["event_storage_service"],
            action_handler_instance=action_handler, person_service=db_services["person_storage_service"],
        )
        ...
        core_logic = CoreLogic(
            core_comm_layer=core_comm_layer, action_handler_instance=action_handler,
            state_manager=state_manager, chat_session_manager=None,
            thought_storage_service=db_services["thought_storage_service"],
            thought_generator=thought_generator, thought_persistor=thought_persistor,
            prompt_builder=prompt_builder, stop_event=stop_event,
        ...
        return ServiceContainer(
            ...
            action_handler=action_handler, intelligent_interrupter=interrupt_model,
            internal_info_builder=internal_info_builder, intrusive_generator=intrusive_generator,
            message_processor=message_processor, prompt_builder=prompt_builder,
            state_manager=state_manager, summarization_service=summarization_service,
            thought_generator=thought_generator, thought_persistor=thought_persistor,
            unread_info_service=unread_info_service, core_comm_layer=core_comm_layer,
            core_logic=core_logic, chat_session_manager=None,
        )
    ...
    def _initialize_llm_clients(self) -> dict:
        ...
        def _create_client(cfg: ModelParams, purpose: str) -> ProcessorClient | None:
            if not cfg or not cfg.provider or not cfg.model_name: return None
            try:
                args = {"model": {"provider": cfg.provider.upper(), "name": cfg.model_name}, **vars(general_llm_settings_obj), **{k: v for k, v in vars(cfg).items() if v is not None and k not in ["provider", "model_name"]}}
        ...
        clients = {"main_consciousness_llm_client": _create_client(models.main_consciousness, "main_consciousness"), "summary_llm_client": _create_client(models.information_summary, "information_summary"), "web_search_agent_client": _create_client(models.web_search_agent, "web_search_agent"), "intrusive_thoughts_llm_client": None, "focused_chat_llm_client": None}
        if config.intrusive_thoughts_module_settings.enabled: clients["intrusive_thoughts_llm_client"] = _create_client(models.intrusive_thoughts, "intrusive_thoughts")
        if config.focus_chat_mode.enabled: clients["focused_chat_llm_client"] = _create_client(models.focused_chat, "focused_chat")
        if not clients["main_consciousness_llm_client"]: raise RuntimeError("主意识LLM客户端初始化失败。")
        if config.focus_chat_mode.enabled and not clients["focused_chat_llm_client"]: raise RuntimeError("专注聊天LLM客户端已启用但初始化失败。")
        logger.info("LLM客户端初始化完毕。")
        return clients

    async def _initialize_database_and_services(self) -> dict:
        conn_manager = await ArangoDBConnectionManager.create_from_config(config.database, core_collection_configs=CoreDBCollections.get_all_core_collection_configs())
        if not conn_manager or not conn_manager.db: raise RuntimeError("数据库连接管理器初始化失败。")
        services_to_create = {"event_storage_service": EventStorageService, "conversation_storage_service": ConversationStorageService, "thought_storage_service": ThoughtStorageService, "action_log_service": ActionLogStorageService, "person_storage_service": PersonStorageService, "summary_storage_service": SummaryStorageService}
        initialized_services = {"conn_manager": conn_manager}
        for instance_name, service_class in services_to_create.items():
            instance = service_class(db_manager=conn_manager) if service_class is SummaryStorageService else service_class(conn_manager=conn_manager)
            if isinstance(instance, Initializable): await instance.initialize_infrastructure()
            initialized_services[instance_name] = instance
        logger.info("所有核心数据存储服务均已初始化。")
        return initialized_services

    async def _initialize_interrupt_model(self, event_storage_service: EventStorageService) -> IntelligentInterrupter:
        ...
        if "default" not in speaker_weights_dict: speaker_weights_dict["default"] = 1.0
        interrupt_model_instance = IntelligentInterrupter(speaker_weights=speaker_weights_dict, objective_keywords=interrupt_config.objective_keywords, core_importance_concepts=interrupt_config.core_importance_concepts, semantic_markov_model=semantic_markov_model)
        logger.info("=== 中断判断模型（小色猫·无状态版）已成功初始化！ ===")
        return interrupt_model_instance

    async def _get_semantic_model(self, event_storage_service: EventStorageService) -> SemanticModel:
        iis_builder_instance = IISBuilder(event_storage=event_storage_service)
